...
def load_vocabulary(vocab_path):
    # Load sign_name -> index mapping from JSON
    with open(vocab_path, 'r') as f:
        sign_to_idx = json.load(f)
    
    # Create reverse mapping: index -> sign_name
    idx_to_sign = {idx: sign for sign, idx in sign_to_idx.items()}
    
    # Add blank token
    BLANK_IDX = len(idx_to_sign)
    idx_to_sign[BLANK_IDX] = "<blank>"
    
    return idx_to_sign, sign_to_idx, BLANK_IDX

# Use it
idx_to_sign, sign_to_idx, BLANK_IDX = load_vocabulary("models/wlasl_model_vocab.json")
